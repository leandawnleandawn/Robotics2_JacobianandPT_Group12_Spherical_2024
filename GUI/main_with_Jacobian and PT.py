...
class RoboticProgram(tk.Tk):
    ...
    def __init__(self):
        ...
        FKin = tk.Button(text = "Forward Kinematics and Jacobian", command = FkinWindow)
        FKin.pack()
        
        IKin = tk.Button(text = "Inverse Kinematics", command= IkinWindow)
        IKin.pack()
        
        PT = tk.Button(text = "Path and Trajectory Planning", command = PathandTrajWindow)
        PT.pack()
...
class PathandTrajWindow(RoboticProgram):
    def __init__(self):
        ...
        PT = tk.LabelFrame(master=self.windowTitle, text = "Program Traj Functions", font=(5))
        PT.grid(row = 0, column = 0)
        
        pick_place = tk.Button(PT, text = "Pick and Place", command= self.pickPlace)
        pick_place.grid(row=0, column=0)
        Welding = tk.Button(PT, text = "Welding" , command=self.welding)
